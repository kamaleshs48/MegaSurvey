...
/**
 * Generated class for the EducationTabPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-education-tab',
  templateUrl: 'education-tab.html',
})
export class EducationTabPage {
  ReligionID: any;
  _List = []
  _RList = []
  ...
  */
  ...
  UpdateRecord(selected, id) {
    try {
      this.sqlite.create({
        ...
        db.executeSql('UPDATE tbl_Education SET  Status = ?  Where ID = ? ', [selected, id])
          .then(res => {
  ...
  BindList() {
    ...
    this.sqlite.create({
      ...
      db.executeSql('select * from tbl_Education AS G  order by G.Name ', [])
        .then(res => {
          this._List = [];

...
/**
 * Generated class for the ProfessionTabPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-profession-tab',
  templateUrl: 'profession-tab.html',
})
export class ProfessionTabPage {
  ReligionID: any;
  _List = []
  _RList = []
  ...
  */
  ...
  UpdateRecord(selected, id) {
    try {
      this.sqlite.create({
        ...
        db.executeSql('UPDATE tbl_Profession SET  Status = ?  Where ID = ? ', [selected, id])
          .then(res => {
  ...
  BindList() {
    ...
    this.sqlite.create({
      ...
      db.executeSql('select * from tbl_Profession AS G  order by G.Name ', [])
        .then(res => {
          this._List = [];

...
export class EducationPage {
  ...
  SaveGender(value, text) {
    ...
    let alert = this.alertCtrl.create({
      title: '',
      mode: 'ios',
      message: "Your Education is '" + text + "', is it right?",
      buttons: [
        {
  ...
  BindList() {
    ...
    this.sqlite.create({
      ...
      db.executeSql('select G.ID, G.Name,LR.Regional_Language ,G.Icons from tbl_Education AS G Left Join tbl_LanguageRegional AS LR ON G.ID=LR.Education_ID AND LR.Language_ID=? WHERE G.Status=1', [this.Language_ID])
        .then(res => {
          this._List = [];
  ...
  UpdateSurveyData_OtherFields(value, SN, Survey_ID) {
    this.sqlite.create({
      ...
      db.executeSql('UPDATE SurveyData SET  Education_ID = ?  ,IsActive=1 Where RowID = ? AND Survey_ID = ?', [value, SN, Survey_ID])
        .then(res => {
  ...
  BindNextPage() {
    ...
    this.sqlite.create({
      ...
      db.executeSql('select Name FROM tbl_PageMaster WHERE ID > 7 AND Status = 1 ORDER BY ID ASC LIMIT 1 ', [])
        .then(res => {

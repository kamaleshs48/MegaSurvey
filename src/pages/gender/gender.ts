...
export class GenderPage {
  ...
  SaveGender(value, text) {
    ...
    let alert = this.alertCtrl.create({
      title: '',
      mode: 'ios',
      message: "Your Gender is '" + text + "', is it right?",
      buttons: [
        {
        ...
        },
        {
          ...
          handler: () => {
            ...
            if (this.NextPage) {
              let contactModal = this.modalCtrl.create(this.NextPage, { Language_ID: this.Language_ID, Survey_ID: this.Survey_ID, SN: this.SN, Survey_Type_ID: this.Survey_Type_ID ,Gender_ID:value});
  ...
  public closeModal() {
    ...
    if (this.NextPage) {
    let contactModal = this.modalCtrl.create(this.NextPage, { Language_ID: this.Language_ID, Survey_ID: this.Survey_ID, SN: this.SN, Survey_Type_ID: this.Survey_Type_ID,Gender_ID:1 });
    contactModal.present();
    }
    setTimeout(() => {
      this.viewCtrl.dismiss();
  ...
  BindList() {
    ...
    this.sqlite.create({
      ...
      db.executeSql('select G.ID, G.Name,LR.Regional_Language ,G.Icons from tbl_Gender AS G Left Join tbl_LanguageRegional AS LR ON G.ID=LR.Gender_ID AND LR.Language_ID=?', [this.Language_ID])
        .then(res => {
          this._List = [];
  ...
  UpdateSurveyData_OtherFields(value, SN, Survey_ID) {
    this.sqlite.create({
      ...
      db.executeSql('UPDATE SurveyData SET  Gender_ID = ?  ,IsActive=1 Where RowID = ? AND Survey_ID = ?', [value, SN, Survey_ID])
        .then(res => {
  ...
  BindNextPage() {
    ...
    this.sqlite.create({
      ...
      db.executeSql('select Name FROM tbl_PageMaster WHERE ID > 5 AND Status = 1 ORDER BY ID ASC LIMIT 1 ', [])
        .then(res => {

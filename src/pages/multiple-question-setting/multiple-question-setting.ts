...
/**
 * Generated class for the MultipleQuestionSettingPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-multiple-question-setting',
  templateUrl: 'multiple-question-setting.html',
})
export class MultipleQuestionSettingPage {
  ReligionID: any;
  Language_ID: 0;
  _List = []
  _RList = []
  ...
  */
  ...
  async Save() {
    ...
    for (var i = 0; i < this._List.length; i++) {
      ...
      await this.UpdateRecord(selected, id, Survey_ID);
    }
  ...
  */
  ...
  UpdateRecord(selected, id, Survey_ID) {
    try {
      this.sqlite.create({
        name: 'surveydb.db',
        location: 'default'
        ...
        db.executeSql('UPDATE tbl_SurveyAssignQuestion SET  Status = ?  Where Question_ID = ? AND  Survey_ID =? ', [selected, id, Survey_ID])
          .then(res => {
  ...
  BindQuestion(Survey_ID) {
    ...
    this.sqlite.create({
     ...
     WHERE QM.Question_Type_ID=2 AND QM.Question_ID=0 AND SQ.Survey_ID=?  ORDER BY QM.ID `, [this.Language_ID, Survey_ID])
        .then(res => {
          this._List = [];

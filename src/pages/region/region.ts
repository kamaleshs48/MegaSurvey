...
export class RegionPage {
  ...
  SaveGender(value, text) {
    ...
    let alert = this.alertCtrl.create({
      title: '',
      mode: 'ios',
      message: "Your Religion is '" + text + "', is it right?",
      buttons: [
        {
        ...
        },
        {
          text: 'Yes',
          handler: () => {
            // this.navCtrl.push('SubCastPage');
            //  this.viewCtrl.dismiss();
            //  this.navCtrl.push('AgeGroupPage');
            this.UpdateSurveyData_OtherFields(value, this.SN, this.Survey_ID);
            let contactModal = this.modalCtrl.create(this.NextPage, { Language_ID: this.Language_ID, Religion_ID: value, Survey_ID: this.Survey_ID, SN: this.SN });
  ...
  public closeModal() {
    ...
    let contactModal = this.modalCtrl.create(this.NextPage, { Language_ID: this.Language_ID, Religion_ID: 0, Survey_ID: this.Survey_ID, SN: this.SN });
    contactModal.present();
    setTimeout(() => {
      this.viewCtrl.dismiss();
    }, 500);
  ...
  BindList() {
    ...
    this.sqlite.create({
      ...
      db.executeSql('select G.ID, G.Name,LR.Regional_Language ,G.Icons from tbl_Religion AS G Left Join tbl_LanguageRegional AS LR ON G.ID=LR.Religion_ID AND LR.Language_ID=? WHERE G.Status=1', [this.Language_ID])
        .then(res => {
          this._List = [];
  ...
  UpdateSurveyData_OtherFields(value, SN, Survey_ID) {
    this.sqlite.create({
      ...
      db.executeSql('UPDATE SurveyData SET  Religion_ID = ? ,IsActive=1 Where RowID = ? AND Survey_ID = ?', [value, SN, Survey_ID])
        .then(res => {
  ...
  BindNextPage() {
    ...
    this.sqlite.create({
      ...
      db.executeSql('select Name FROM tbl_PageMaster WHERE ID > 9 AND Status = 1 ORDER BY ID ASC LIMIT 1 ', [])
        .then(res => {

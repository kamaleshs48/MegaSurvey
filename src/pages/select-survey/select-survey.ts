...
export class SelectSurveyPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams,
    ...
    setTimeout(() => {
      this.GetSurveyData();
      this.GetSurveyMaxLimit(1, 1);
      this.GetSurveyPresentLimit(1, 1);
      ...
      db.executeSql('SELECT MaxLimit FROM SurveyMaster WHERE UserID = ' + UserID + ' AND ID = ' + SurveyID, [])
        .then(res => {
          if (res.rows.length > 0) {
            this.MaxLimit = res.rows.item(0).MaxLimit;
            //  alert('MaxLimit' + this.MaxLimit);
            return MaxLimit;
          }
  ...
  GetSurveyPresentLimit(SurveyID, UserID) {
    ...
    this.sqlite.create({
      ...
      db.executeSql('SELECT PresentLimit FROM SurveyMaster WHERE UserID = ' + UserID + ' AND ID = ' + SurveyID, [])
        .then(res => {
          if (res.rows.length > 0) {
            this.PresentLimit = res.rows.item(0).PresentLimit;
  ...
  PostDataToServer() {
    ...
    //  this.user.ShowAlert(this.SurveyList[this.SurveyList.length - 1].MQRList.length);
    // this.SurveyList[0].MQRList = this.MQRList;
